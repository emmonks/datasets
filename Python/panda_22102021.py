...
#df2=df[(df['Flow Duration'] > 117000000)]

# Tiktok
#df2=df[(df['Src IP'] == '192.168.254.103') & (df['Flow Duration'] > 100000000) & (df['Dst Port'] == 443) & (df['Protocol'] == 17) ]
#df2=df[(df['Src IP'] == '192.168.254.103') & (df['Volume'] > 750000) & (df['Dst Port'] == 443) & (df['Protocol'] == 17) ]
#df2=df[(df['Flow Duration'] > 30000000) & (df['Total Bwd packets'] > 500)]
#df2=df[(df['Volume'] > 800000)]

# Instagram
#df2=df[(df['Src IP'] == '2804:04B0:10AA:2500:9C27:0700:A335:B1F0') & (df['Dst Port'] == 443) & (df['Protocol'] == 17) & (df['Volume'] > 750000) ]
#(df['Fwd Packet Length Mean'] > 91) ]
df2=df[(df['Volume'] > 3000000)]
...
# CicFlow - 29012022
#2804:04B0:10AA:2500:020C:29FF:FE64:3255
# Facebook
#df2=df[(df['Flow Duration'] > 100000) & (df['Dst Port'] == 443) & (df['Protocol'] == 17) ]

# Youtube App Tablet
#df2=df[((df['Src IP'] == '2804:04B0:10AA:2500:0D4A:2E46:9E2F:C02D') | (df['Dst IP'] == '2804:04B0:10AA:2500:0D4A:2E46:9E2F:C02D')) & ((df['Dst Port'] == 443) | (df['Src Port'] == 443 )) & (df['Protocol'] == 17) & (df['Volume'] > 75000)]
#df2=df[(df['Flow Duration'] > 118999998)]
#df2=df[(df['Volume'] > 6000000)]

# Downloads
#df2=df[(df['Total Bwd packets'] > 1000)]

# Volume
#df2=df[(df['Volume'] > 7500000)]
...
result = df2.groupby('Label').agg({'Bwd Packet/Bulk Avg': ['max']}).iloc[0]['Bwd Packet/Bulk Avg']['max']
max_Bwd_Packet_Bulk_Avg = 150000
print result

result = df2.groupby('Label').agg({'Fwd Packet/Bulk Avg': ['max']}).iloc[0]['Fwd Packet/Bulk Avg']['max']
max_Fwd_Packet_Bulk_Avg = result
...
# Atributos
df2['NormFwd_Packet_Length_Mean'] = np.clip(((df2['Fwd Packet Length Mean'] / max_Fwd_Packet_Length_Mean) * 10),0,10)
# Limita em 10 o max
...
df2['NormFwd_Packet_Length_Std'] =  np.clip(((df2['Fwd Packet Length Std'] / max_Fwd_Packet_Length_Std) * 10),0,10)
df2['NormFlow_IAT_Std'] = np.clip(((df2['Flow IAT Std'] / max_Flow_IAT_Std) * 10),0,10)
df2['NormFwd_IAT_Std'] = (df2['Fwd IAT Std'] / max_Fwd_IAT_Std) * 10
df2['NormBwd_IAT_Total'] = np.clip(((df2['Bwd IAT Total'] / max_Bwd_IAT_Total) * 10),0,10)

df2['NormBwd_IAT_Std'] = np.clip(((df2['Bwd IAT Std'] / max_Bwd_IAT_Std) * 10),0,10)

df2['NormBwd_Bulk_Rate_Avg'] = (df2['Bwd Bulk Rate Avg'] / max_Bwd_Bulk_Rate_Avg) * 10

df2['NormBwd_Packet_Bulk_Avg'] = np.clip(((df2['Bwd Packet/Bulk Avg'] / max_Bwd_Packet_Bulk_Avg) * 10),0,10)
df2['NormFwd_Packet_Bulk_Avg'] = (df2['Fwd Packet/Bulk Avg'] / max_Fwd_Packet_Bulk_Avg) * 10
...
grouped_single32 = df2.groupby('Label').agg({'NormBwd_Packet_Bulk_Avg': ['mean', 'min', 'max', 'std']})
grouped_single33 = df2.groupby('Label').agg({'NormFwd_Packet_Bulk_Avg': ['mean', 'min', 'max', 'std']})
...
print(grouped_single32)
print(grouped_single33)
